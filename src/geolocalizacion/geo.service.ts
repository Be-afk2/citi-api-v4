...
export class GeoService {
  ...
  async GetGeoData(data: GeoDataDto) {
    ...
    return {
      ...
      ciudad: response.data.results[0].components.city
  ...
  async GetData(data: GeoDataDto) {
    ...
    const ciudad = await this.crearCiudad(
      geodata.ciudad,
      region,
      geodata.codigoPostal,
    );
  ...
  async crearRegion(RegionName: string, pais: Pais) {
    let Region = await this.RegionRepository.findOneBy({
      nombre: RegionName,
      Pais: { id: pais.id },
    });
    if (Region) {
  ...
  async crearPais(PaisName: string) {
    let Pais = await this.PaisRepository.findOneBy({ nombre: PaisName });
  ...
  async crearCiudad(CiudadName: string, Region: Region, PostalCode: string) {
    let Ciudad = await this.CiudadRepository.findOneBy({
      nombre: CiudadName,
      region: { id: Region.id },
    });
    if (Ciudad) {
    ...
    if (PostalCode) {
      Ciudad.CodigoPostal = parseInt(
        PostalCode.split(' ')[0] + '0'.repeat(PostalCode.split(' ')[0].length),
      );
    }
  ...
  async GetUserDataGeo(userId: GetOneDto) {
    ...
    const data = await this.GeoDataRepository.createQueryBuilder('GeoData')
